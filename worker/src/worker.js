...
// The listing-relevance gate (isAccessory / modelToken / matchesModel /
// keepListing) lives in ../../shared/relevance.js — one copy, shared verbatim
// with the homeserver scraper so the two runtimes can't drift. Wrangler bundles
// this relative import via esbuild.
import { isAccessory, modelToken, matchesModel, keepListing } from "../../shared/relevance.js";
...
async function redditSearch(sub, q, min, max, now) {
  ...
  return entries.map(function (block) {
    ...
    return keepListing(l.title, l.price, { model: model, min: min, max: max });
  }).slice(0, 12);
}
